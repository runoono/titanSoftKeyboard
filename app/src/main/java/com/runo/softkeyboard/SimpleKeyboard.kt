...
class SimpleKeyboard {
    ...
    val showNumbersRow: Boolean = true
...
//var mShiftState = ShiftState.OFF
    ...
    private var mCtrlState = CtrlState.OFF
    ...
    class Row {
        ...
        var parent: SimpleKeyboard

        var isNumbersRow: Boolean = false

        constructor(parent: SimpleKeyboard) {
            this.parent = parent
        }

        constructor(res: Resources, parent: SimpleKeyboard, parser: XmlResourceParser?) {
            this.parent = parent
            val a = res.obtainAttributes(Xml.asAttributeSet(parser), R.styleable.SimpleKeyboard)
            defaultWidth = getDimensionOrFraction(a, R.styleable.SimpleKeyboard_keyWidth, parent.mDisplayWidth, parent.mDefaultWidth)
            defaultHeight = (res.getDimension(R.dimen.key_height) * this.parent.mKeyboardHeightMultiplier).roundToInt()
            defaultHorizontalGap = getDimensionOrFraction(a, R.styleable.SimpleKeyboard_horizontalGap, parent.mDisplayWidth, parent.mDefaultHorizontalGap)
            isNumbersRow = a.getBoolean(R.styleable.SimpleKeyboard_isNumbersRow, false)
            a.recycle()
        }
    }
    ...
    class Key(parent: Row) {
        ...
        constructor(res: Resources, parent: Row, x: Int, y: Int, parser: XmlResourceParser?) : this(parent) {
            this.x = x
            this.y = y
            var a = res.obtainAttributes(Xml.asAttributeSet(parser), R.styleable.SimpleKeyboard)
            width = getDimensionOrFraction(a, R.styleable.SimpleKeyboard_keyWidth, keyboard.mDisplayWidth, parent.defaultWidth)
            height = parent.defaultHeight
            gap = getDimensionOrFraction(a, R.styleable.SimpleKeyboard_horizontalGap, keyboard.mDisplayWidth, parent.defaultHorizontalGap)
            this.x += gap

            a.recycle()
            a = res.obtainAttributes(Xml.asAttributeSet(parser), R.styleable.SimpleKeyboard_Key)

            label = a.getText(R.styleable.SimpleKeyboard_Key_keyLabel) ?: ""
            code = a.getInt(R.styleable.SimpleKeyboard_Key_code, 0)

            if (label.isNotEmpty() && code == 0) {
                code = label[0].code
            }

            popupCharacters = a.getText(R.styleable.SimpleKeyboard_Key_popupCharacters)
            popupResId = a.getResourceId(R.styleable.SimpleKeyboard_Key_popupKeyboard, 0)
            repeatable = a.getBoolean(R.styleable.SimpleKeyboard_Key_isRepeatable, false)
            edgeFlags = a.getInt(R.styleable.SimpleKeyboard_Key_keyEdgeFlags, 0)
            icon = a.getDrawable(R.styleable.SimpleKeyboard_Key_keyIcon)
            icon?.setBounds(0, 0, icon!!.intrinsicWidth, icon!!.intrinsicHeight)

            secondaryIcon = a.getDrawable(R.styleable.SimpleKeyboard_Key_secondaryKeyIcon)
            secondaryIcon?.setBounds(0, 0, secondaryIcon!!.intrinsicWidth, secondaryIcon!!.intrinsicHeight)

            topSmallNumber = a.getString(R.styleable.SimpleKeyboard_Key_topSmallNumber) ?: ""
    ...
    constructor(context: Context, @XmlRes xmlLayoutResId: Int, enterKeyType: Int) {
        ...
        mKeyboardHeightMultiplier = KEYBOARD_HEIGHT_100_PERCENT.toFloat() //getKeyboardHeightMultiplier(context.config.keyboardHeightPercentage)
        mKeys = ArrayList()
...
//}
    ...
    fun setCtrlState(ctrlState: CtrlState): Boolean {
        if (this.mCtrlState != ctrlState) {
            this.mCtrlState = ctrlState
            return true
        }
        return false;
    }
...
//                        }
                        ...
                        TAG_KEY -> {
                            if (currentRow?.isNumbersRow == true && showNumbersRow) {
                                continue
                            }
                            inKey = true
                            key = createKeyFromXml(res, currentRow!!, x, y, parser)
                            if (true /*context.config.showNumbersRow*/) {
                            ...
                            if (key.code == KEYCODE_ENTER) {
                                val enterResourceId = R.drawable.enter_key
    ...
    private fun parseKeyboardAttributes(res: Resources, parser: XmlResourceParser) {
        val a = res.obtainAttributes(Xml.asAttributeSet(parser), R.styleable.SimpleKeyboard)
        mDefaultWidth = getDimensionOrFraction(a, R.styleable.SimpleKeyboard_keyWidth, mDisplayWidth, mDisplayWidth / 10)
        mDefaultHeight = res.getDimension(R.dimen.key_height).toInt()
        mDefaultHorizontalGap = getDimensionOrFraction(a, R.styleable.SimpleKeyboard_horizontalGap, mDisplayWidth, 0)
        a.recycle()
    }
